...
import { pavementOffsetFor, pavementPaths, roadThrough } from "@/tiles/footway";
  ...
  it("moves no pavement — the band still hugs the same kerb", () => {
    ...
    for (const id of Object.keys(base)) {
      ...
      for (const side of [1, -1] as const) {
        expect(
          pavementOffsetFor(withKerb[id], side, through.from, through.to),
          `the walkers' pavement on ${id} moved`,
        ).toBe(pavementOffsetFor(base[id], side, through.from, through.to));
      }
