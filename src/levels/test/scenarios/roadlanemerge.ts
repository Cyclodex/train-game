...
// increase/decrease. Cars spawn on the 1- or 2-lane edges and ride the kerb
// lane, so the inner lanes render but stay empty — the sim only clamps inward
// at a narrowing and has no "spread outward into a new lane" behaviour yet
// (that lands later). The original clamp demo (a wide-at-edge merge squeezing a
// lane-1 car into lane 0) is intentionally not reproduced here.
...
export const roadlanemerge: TestScenario = {
  ...
  description:
    "Four straight roads, one per row, showing every lane-count transition between 1, 2 and 3 lanes (step up, step down, skip-a-lane, and a full sweep). Each narrowing paints Swiss-style lane-drop arrows (2 advance + 1 at the seam) in the ending lane. A geometry gallery — inner lanes render but stay empty for now; traffic rides the kerb lane.",
  level: {
