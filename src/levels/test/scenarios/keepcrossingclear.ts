...
// between two mismatched depots on crossing B's rail (3,0 green ↔ 3,2 blue), so B's
// gate keeps closing and the eastbound car queue backs up toward A. Only one car
// fits in the gap between the crossings; the next car would have to stop with its
// body on crossing A — so instead it holds short of A, leaving the tracks clear,
// until B opens and the queue flushes. Crossing A's own gate never closes (no train
// runs its rail), proving the hold is about the jam beyond, not the gate.
export const keepcrossingclear: TestScenario = {
  ...
  level: {
    ...
    "3,2": expandKind("depot", 0), // opens north
  ...
  trains: {
    train1: mkTrain("train1", 3, 0, "people", 2, "3,2"),
  },
  colors: {
    depotColors: {
      "3,0": "green", // mismatch → train bounces back out
      "3,2": "blue", // mismatch → train bounces straight back
    },
    trainColors: {
