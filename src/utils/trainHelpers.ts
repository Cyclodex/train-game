import { Coordinates, TrainDirection, TrainObject } from "@/types";
import { getCoordinatesId } from "./tileHelpers";

export function getTrainDirection(train: TrainObject, origin: Coordinates) {
  const x = train.x - origin.x;
  const y = train.y - origin.y;
